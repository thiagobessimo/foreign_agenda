...
import theme from "../../themes/brutalBlue"
...
const Copy: React.FC<CopyProps> = ({ toCopy }) => {
  ...
  return (
    <CopyButton onClick={copyToClipboardOnClick} data-a11y="false">
      {hasCopied ? (
        <>
          Copied <Icons.Copied fill="#c1c2cc" />
        </>
      ) : (
        <>
          Copy <Icons.Copy fill="#c1c2cc" />
        </>
      )}
    </CopyButton>
  );
    ...
    return (
      <Container>
        <LiveProvider code={codeString} noInline={true} theme={theme}>
          <LiveEditor style={{ marginBottom: "3px", borderRadius: "0px" }} />
          <LivePreview style={{ fontSize: "14px", borderRadius: "0px" }} />
          <LiveError style={{ color: "#ef5050" }} />
        </LiveProvider>
      </Container>
    );
  } else {
